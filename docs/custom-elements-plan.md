...
**Phase 1 (now):** Everything in a single binary. Custom elements are separate files but compiled into the same `@solo/native` crate. This keeps the build simple while we validate the architecture.
